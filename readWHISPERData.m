function readWHISPERData(binName, path, chList, chSaveNames)
...
if dataType == 'A'
    ...
    saveTrodes(dataArray,chList,chSaveNames,true,[300 str2double(meta.niSampRate)],meta);
else
...
function saveTrodes(dataArray,chList,chSaveNames,bandPassBeforeSaving,bp_range,meta)

disp(['There are ' num2str(size(dataArray,1)) ' channels saved in this .bin file.']);

for i=1:length(chList)
    temp=dataArray(chList(i),:);
    if bandPassBeforeSaving==true
        temp=bandPassLFP(temp,str2double(meta.niSampRate),bp_range(1),bp_range(2),0);
    end
    data.Values=temp;
    data.ADFreq=str2double(meta.niSampRate);
    save(chSaveNames{i},'data');
end
